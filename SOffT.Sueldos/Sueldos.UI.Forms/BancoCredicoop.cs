...
namespace Sueldos.View
{
	...
	static class BancoCredicoop
	{
		...
		{
			public static string tipoCuentaCredito = "";
			//char(1) 1:ctacte - 2:caja ahorro
			public const string banco = "191";
			//char(3) codigo de banco
			public static string sucursalCuentaCredito = "";
			//char(3) numero de sucursal del banco
			public static string cuentaCredito = "";
			//char(7) 6 cuenta + 1 verificador
			public static string controlDeCuenta;
			//char(1) repite digito verificador de cuenta
			public static string importeDeCredito = "";
			//char(15) 13 enteros y 2 decimales
			public static string beneficiario = "";
			//char(40) apellido y nombre del beneficiario
			public static string cuil = "";
			//char(11) cuil del beneficiario
			public static string legajo = "";
			//char(10) legajo asignado por la empresa
		...
		/// <param name = "tiposLiquidacionesSeleccionados"></param>
		/// <param name = "fechaAcreditacion"></param>
		/// <param name = "todosLosConveios"></param>
		...
		public static void generaArchivo (int idliquidacion, string archivo, List<int> tiposLiquidacionesSeleccionados, DateTime fechaAcreditacion, bool todosLosConveios, int idConvenio)
		{
			...
			if (todosLosConveios)
				rsLegajos = DB.Instancia.SPToDbDataReader ("liquidacionesNetosPorLegajoPorBanco", "@idLiquidacion", idliquidacion, "@idFormaDePago", 2, "@idBanco", 3);
			else
				rsLegajos = DB.Instancia.SPToDbDataReader ("liquidacionesNetosPorLegajoPorBancoPorConvenio", "@idLiquidacion", idliquidacion, "@idFormaDePago", 2, "@idBanco", 3, "@idConvenio", idConvenio);

			while (rsLegajos.Read ()) {  //para cada legajo recorro todos los campos definidos en tablas
				if (Varios.Right (DB.Instancia.SPToScalar ("empleadosSueldosConsultarValorLegajo", "legajo", Convert.ToInt32 (rsLegajos ["legajo"]), "codigo", 21).ToString (), 1) == "1")
					movimiento.tipoCuentaCredito = "2";
				else
					movimiento.tipoCuentaCredito = "1";
				movimiento.sucursalCuentaCredito = Varios.Left (DB.Instancia.SPToScalar ("empleadosSueldosConsultarValorLegajo", "legajo", Convert.ToInt32 (rsLegajos ["legajo"]), "codigo", 21).ToString ().PadLeft (4, '0'), 3).PadLeft (3, '0');
				movimiento.cuentaCredito = Varios.Left (DB.Instancia.SPToScalar ("empleadosSueldosConsultarValorLegajo", "legajo", Convert.ToInt32 (rsLegajos ["legajo"]), "codigo", 22).ToString (), 7).PadLeft (7, '0');
				...
				movimiento.cuil = Varios.Left (DB.Instancia.SPToScalar ("empleadosSueldosConsultarValorLegajo", "legajo", Convert.ToInt32 (rsLegajos ["legajo"]), "codigo", 18).ToString ().Replace ("-", ""), 11);
		...
		public static void generaArchivoDesdeAnticipos (int anioMes, int idTipoAnticipo, string archivo, DateTime fechaAcreditacion, bool todosLosConveios, int idConvenio)
		{
			...
			//repoteLiquidacionesNetoPorLegajo. calcular haberes+adicionales-retenciones
			if (todosLosConveios)
				rsLegajos = DB.Instancia.SPToDbDataReader ("anticiposConsultarParaAcreditarPorBanco", "@anioMes", anioMes, "@idTipoAnticipo", idTipoAnticipo, "@idBanco", 3);
			else
				rsLegajos = DB.Instancia.SPToDbDataReader ("anticiposConsultarParaAcreditarPorBancoPorConvenio", "@anioMes", anioMes, "@idTipoAnticipo", idTipoAnticipo, "@idBanco", 3, "@idConvenio", idConvenio);
