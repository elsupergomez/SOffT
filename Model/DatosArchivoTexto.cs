...
namespace Model
{
	public class DatosArchivoTexto
	{
		...
		public static void GeneraArchivoTxT (string nomArchi, IDataReader dr, Boolean FiltraCaracteres)
		{
			...
			while (dr.Read ()) {
				...
				for (int i = 0; i < dr.FieldCount; i++) {
					try {
						cadenita = dr.GetString (i);
					} catch {
						cadenita = string.Empty;
					}
		...
		public static void vaciarArchivo (string archivo)
		{
			...
			} //no hace nada no existe el archivo
		}
		...
		public static DataTable Lee_CSV (string nombre, string ruta)
		{
			...
			try {
				...
				//MyLog4Net.Instance.getCustomLog("DatosArchivoTexto.cs").Error("Lee_CSV() -> Ruta: " + ruta +                                       " Archivo: " + nombre + " mensaje: " + ex.Message, ex); 
				return dtCSV;
			}
			return dtCSV;
		}
